...
def save_and_rename(grid_folder, new_filename):
    subfolders = [f.path for f in os.scandir(grid_folder) if f.is_dir()]
    latest_folder = max(subfolders, key=os.path.getmtime)

    tif_file = glob.glob(os.path.join(latest_folder, "*.tiff"))[0]

    new_path = os.path.join(grid_folder, new_filename)

    shutil.move(tif_file, new_path)
    shutil.rmtree(latest_folder)

def download_s2(grid_folder, bbox, size_s2, grid_id):

    # NDWI
    req_ndwi = SentinelHubRequest(
        evalscript=EVALSCRIPT_S2_NDWI,
    ...
    req_ndwi.get_data(save_data=True)
    save_and_rename(grid_folder, f"grid{grid_id}_S2_NDWI.tif")
    ...
    save_and_rename(grid_folder, f"grid{grid_id}_S2_RGB.tif")

def download_s1(grid_folder, bbox, size_s1, grid_id):
    ...
    save_and_rename(grid_folder, f"grid{grid_id}_S1_SAR_GRD.tif")
...
def main():
    for _, row in df.iterrows():
        ...
        try:
            download_s2(grid_folder, bbox, size_s2, grid_id)
            download_s1(grid_folder, bbox, size_s1, grid_id)
